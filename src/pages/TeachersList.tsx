...
const TeachersList = () => {
  ...
  const fetchTeachers = async () => {
    ...
    try {
      ...
      if (loaded === null && settings.data_guru && Array.isArray(settings.data_guru)) {
          ...
          <div className="max-w-6xl mx-auto space-y-4">
            <div className="flex items-center gap-2">
              <Button
                ...
                variant="outline"
                className="bg-white/10 hover:bg-white/20 text-white border-white/20 rounded-xl gap-2 text-xs font-bold -ml-2 h-9 px-3.5"
              >
                <ArrowLeft className="w-4 h-4 text-emerald-300" /> Kembali ke Ruang Guru
              </Button>
              <Button
                onClick={() => navigate('/')}
                variant="ghost"
                className="text-emerald-200 hover:text-white hover:bg-white/10 rounded-xl gap-1.5 text-xs font-semibold h-9 px-3"
              >
            ...
            <CardContent className="p-4 space-y-4">
              <div className="flex flex-col md:flex-row items-center gap-3">
                ...
                <div className="flex items-center gap-2 w-full md:w-auto">
                  ...
                  <div className="flex items-center gap-2">
                    <PrintSecurityIndicator documentTitle="Daftar Guru & Tenaga Kependidikan (GTK)" />
                    <Button
                      onClick={handlePrint}
                      className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl text-xs font-bold h-9 px-4 gap-1.5 shadow-md cursor-pointer"
                    >
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredTeachers.map((teacher, index) => (
                <Card key={teacher.id} className="border-0 shadow-lg rounded-3xl overflow-hidden bg-white hover:shadow-xl transition-shadow p-5 space-y-3">
                  <div className="flex items-center gap-3">
                    {teacher.foto_url ? (
                      <img src={formatImageUrl(teacher.foto_url)} alt={teacher.nama} className="w-14 h-14 rounded-2xl object-cover border-2 border-emerald-200 shadow-sm shrink-0" />
                    ) : (
                  ...
                  <div className="space-y-1 text-xs">
                    ...
                    {teacher.mengajar_kelas && (
                      <div className="flex justify-between items-center pt-0.5">
                        <span className="text-slate-500 font-medium">Mengajar:</span>
