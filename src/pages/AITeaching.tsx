...
const AITeaching = () => {
  ...
  if (previewItem) {
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex flex-col">
        <div className="sticky top-0 z-50 bg-white border-b p-4 flex justify-between items-center print:hidden gap-3 flex-wrap">
          <Button variant="ghost" onClick={() => setPreviewItem(null)} className="font-bold">
            <ArrowLeft className="w-4 h-4 mr-2" /> Kembali
          </Button>
          <div className="flex items-center gap-2.5">
            <PrintSecurityIndicator 
              documentTitle={`Perangkat Pembelajaran AI: ${previewItem.judul || previewItem.mata_pelajaran || 'Dokumen'}`} 
            />
            <Button 
              onClick={() => {
                requirePrintAuth(() => {
                  window.print();
                }, `Perangkat Pembelajaran AI: ${previewItem.judul || previewItem.mata_pelajaran || 'Dokumen'}`);
              }} 
              className="bg-emerald-600 hover:bg-emerald-700 text-white px-8 font-bold cursor-pointer shadow-md"
            >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50/30 flex flex-col items-center py-10 px-4 sm:px-6">
      {/* Top Back Navigation Bar */}
      <div className="w-full max-w-5xl mb-8 flex flex-wrap items-center justify-between gap-3 bg-white/90 backdrop-blur-md p-3.5 sm:px-5 rounded-2xl border border-slate-200/90 shadow-sm">
