    ...
    if (printMode === 'emis_complete') {
      ...
      return (
        <div className="min-h-screen bg-white p-0">
          ...
          <div 
            className="mx-auto print:w-full flex flex-col font-serif p-6"
            style={{ 
              width: '297mm', 
              minHeight: '210mm',
              boxSizing: 'border-box'
          ...
          >
            ...
            <div className="text-center my-4">
              <h2 className="text-lg font-bold underline uppercase tracking-tight">
                DAFTAR DATA SISWA LENGKAP STANDAR EMIS (EDUCATION MANAGEMENT INFORMATION SYSTEM)
              </h2>
              <p className="mt-0.5 text-xs font-bold uppercase text-slate-800">
                KEMENTERIAN AGAMA REPUBLIK INDONESIA - TAHUN PELAJARAN {selectedAcademicYear}
              </p>
              <div className="flex items-center justify-center gap-4 text-[9pt] font-semibold mt-1">
            ...
            <table className="w-full border-collapse border border-black text-[7.5pt]">
              <thead>
                <tr className="bg-gray-100 font-bold text-center">
                  <th className="border border-black p-1 w-6 align-middle">No</th>
                  <th className="border border-black p-1 text-center w-20 align-middle">NISN</th>
                  <th className="border border-black p-1 text-center w-24 align-middle">NIK (No. KTP)</th>
                  <th className="border border-black p-1 text-left min-w-[130px] align-middle">Nama Lengkap Siswa</th>
                  <th className="border border-black p-1 text-center w-8 align-middle">L/P</th>
                  <th className="border border-black p-1 text-center w-12 align-middle">Rombel</th>
                  <th className="border border-black p-1 text-left min-w-[100px] align-middle">Tempat, Tanggal Lahir</th>
                  <th className="border border-black p-1 text-left min-w-[110px] align-middle">Nama Orang Tua</th>
                  <th className="border border-black p-1 text-left min-w-[90px] align-middle">Pekerjaan Ortu</th>
                  <th className="border border-black p-1 text-left min-w-[120px] align-middle">Alamat Domisili</th>
                  <th className="border border-black p-1 text-center w-20 align-middle">No. HP / WA</th>
                  <th className="border border-black p-1 text-center w-20 align-middle">KIP / PIP</th>
                  <th className="border border-black p-1 text-center w-16 align-middle">Status</th>
                </tr>
              </thead>
              <tbody>
                {emisStudents.map((s, idx) => {
                  ...
                  return (
                    <tr key={s.id} className="odd:bg-white even:bg-slate-50">
                      ...
                      <td className="border border-black p-1 text-center font-mono font-semibold">{s.nisn || '-'}</td>
                      <td className="border border-black p-1 text-center font-mono text-[7pt]">{s.nik || '-'}</td>
                      ...
                      <td className="border border-black p-1 text-center font-bold uppercase">{displayRombel}</td>
                      <td className="border border-black p-1 text-left">
                        {s.tempat_lahir ? `${s.tempat_lahir}, ` : ''}{s.tanggal_lahir || '-'}
                      </td>
                      <td className="border border-black p-1 text-left">
                        <div>A: {s.nama_ayah || '-'}</div>
                        <div className="text-[7pt] text-gray-700">I: {s.nama_ibu || '-'}</div>
                      </td>
                      <td className="border border-black p-1 text-left text-[7pt]">
                      ...
                      <td className="border border-black p-1 text-center font-mono text-[7pt]">
                        {s.phone || s.no_hp_ortu || '-'}
                      </td>
                      <td className="border border-black p-1 text-center text-[7pt]">
                        {s.nomor_kip_pip ? (
                          ...
                          <span className="font-bold">Penerima KIP</span>
                        ) : (
                          '-'
                        )}
                      </td>
                      <td className="border border-black p-1 text-center uppercase font-semibold text-[7pt]">
            ...
            <div className="mt-6 flex justify-between items-start text-[8pt]">
              <div className="text-left text-gray-600 max-w-sm italic">
                * Dokumen ini dicetak otomatis melalui Sistem Informasi Akademik Madrasah (SIAKAD) Standar EMIS Kemenag RI.
              </div>
              <PenandatanganDokumen showGuru={false} />
            </div>
          </div>
          <style dangerouslySetInnerHTML={{ __html: `@media print { @page { size: A4 landscape; margin: 6mm; } body { background: white; } .print\\:hidden { display: none; } }` }} />
        </div>
      );
    }
                ...
                <SelectContent className="rounded-xl">
                  ...
                  {sortedClassesList.map((c) => (
                    <SelectItem key={c.id} value={c.id} className="text-sm font-semibold">
                      {c.nama_kelas} (Tingkat {c.tingkat}) - Wali: {c.wali_kelas || '-'}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
