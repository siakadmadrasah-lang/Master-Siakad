...
import { Loader2, Printer, Users, Archive, ArrowLeft, Home } from 'lucide-react';
...
const ExamCards: React.FC = () => {
  ...
  const officialSchoolName = getOfficialMadrasahName(activeMadrasah, siteSettings.identitas_madrasah, 'MADRASAH IBTIDAIYAH');
  ...
  const handlePrint = () => {
    requirePrintAuth(() => {
      window.print();
    }, `Kartu Peserta Ujian / TKAD (${participants.length} Siswa)`);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 print:bg-white print:p-0">
      ...
      <div className="pt-24 pb-12 px-4 md:px-8 max-w-7xl mx-auto print:p-0 print:pt-0 print:max-w-none space-y-6">
        {/* Top Back Navigation Bar */}
        <div className="flex flex-wrap items-center justify-between gap-3 bg-white p-3.5 sm:px-5 rounded-2xl border border-slate-200/90 shadow-sm print:hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 print:hidden">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <PrintSecurityIndicator documentTitle="Kartu Peserta Ujian TKAD" />
              <Button onClick={handlePrint} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl px-8 h-12 shadow-lg shadow-emerald-200 transition-all hover:-translate-y-1 w-full sm:w-auto cursor-pointer">
            ...
            {participants.map((p, index) => (
              <div key={p.id || index} className="flex justify-center print:break-inside-avoid">
                <ExamCard
                  ...
                  headerPengayom={settings.headerPengayom}
                  namaYayasan={settings.namaYayasan}
                  lembagaNama={settings.lembagaNama || officialSchoolName}
                  subHeader={settings.subHeader}
                  alamatKop={settings.alamatKop}
                  kontakKop={settings.kontakKop}
                  lembagaLogo={settings.lembagaLogo}
                  logoKananUrl={settings.logoKananUrl}
                  garisStyle={settings.garisStyle}
