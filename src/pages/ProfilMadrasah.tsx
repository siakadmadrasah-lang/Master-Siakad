...
export default function ProfilMadrasah() {
  ...
  const handlePrint = () => {
    requirePrintAuth(() => {
      window.print();
    }, `Profil Lengkap Kelembagaan - ${identitas.nama_madrasah || 'Madrasah'}`);
  };
      ...
      <main className="flex-1 pt-24 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 print:hidden">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <PrintSecurityIndicator documentTitle="Profil Lengkap EMIS Madrasah" />
            <Button
              onClick={handlePrint}
              className="bg-slate-900 hover:bg-slate-800 text-white font-extrabold rounded-2xl px-5 h-11 shadow-lg shadow-slate-900/10 gap-2 text-xs cursor-pointer"
            >
