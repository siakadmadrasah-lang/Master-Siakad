...
const Penandatangan = () => {
  ...
  const { getScopedKey } = useMadrasah();
  ...
  const fetchPenandatangan = async () => {
    try {
      const storageKey = getScopedKey ? getScopedKey('penandatangan') : 'penandatangan';
      const { data } = await supabase.from('site_settings').select('*').in('id', [storageKey, 'penandatangan']);
      if (data && data.length > 0) {
        const foundScoped = data.find(item => item.id === storageKey);
        const foundBase = data.find(item => item.id === 'penandatangan');
        const activeData = foundScoped?.value || foundBase?.value;
        if (activeData) {
          setPenandatangan(prev => ({ ...prev, ...activeData }));
        }
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const normalizedPenandatangan = {
        ...penandatangan,
        kepala_nama: formatFullNameWithTitles(penandatangan.kepala_nama),
        ketua_panitia_nama: formatFullNameWithTitles(penandatangan.ketua_panitia_nama),
        guru_kelas: (penandatangan.guru_kelas || []).map(g => ({
          ...g,
          nama: formatFullNameWithTitles(g.nama)
        }))
      };

      const storageKey = getScopedKey ? getScopedKey('penandatangan') : 'penandatangan';
      await supabase.from('site_settings').upsert({ id: 'penandatangan', value: normalizedPenandatangan, updated_at: new Date().toISOString() });
      if (storageKey !== 'penandatangan') {
        await supabase.from('site_settings').upsert({ id: storageKey, value: normalizedPenandatangan, updated_at: new Date().toISOString() });
      }
      setPenandatangan(normalizedPenandatangan);
      await refreshSettings();
      showSuccess('Data penandatangan berhasil disimpan dan diformat sesuai ketentuan!');
    } catch (error) { showError('Gagal menyimpan'); } finally { setSaving(false); }
  };

  const handleAddGuru = () => {
    const formattedNama = formatFullNameWithTitles(guruForm.nama);
    const newGuru = { ...guruForm, nama: formattedNama, id: editingGuru?.id || Date.now().toString() };
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card className="border-0 shadow-lg">
          ...
          <CardContent className="p-6 space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">Nama Kepala Madrasah (Otomatis EYD/Gelar)</label>
              <Input 
                placeholder="Contoh: Siti Rochimah, S.Pd.I" 
                value={penandatangan.kepala_nama} 
                onChange={e => setPenandatangan({...penandatangan, kepala_nama: e.target.value})} 
                onBlur={e => {
                  if (e.target.value.trim()) {
                    setPenandatangan({...penandatangan, kepala_nama: formatFullNameWithTitles(e.target.value)});
                  }
                }}
                className="rounded-xl" 
              />
              <p className="text-[10px] text-emerald-600 mt-1">✓ Gelar akademik (S.Pd.I, M.Pd, dsb.) diformat otomatis sesuai EYD/Kemenag.</p>
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">NIP / NPK Kepala</label>
              <Input placeholder="NIP Kepala (atau - jika belum ada)" value={penandatangan.kepala_nip} onChange={e => setPenandatangan({...penandatangan, kepala_nip: e.target.value})} className="rounded-xl" />
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-0 shadow-lg">
          ...
          <CardContent className="p-6 space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">Nama Ketua Panitia (Otomatis EYD/Gelar)</label>
              <Input 
                placeholder="Contoh: Ahmad Fauzi, S.Pd" 
                value={penandatangan.ketua_panitia_nama} 
                onChange={e => setPenandatangan({...penandatangan, ketua_panitia_nama: e.target.value})} 
                onBlur={e => {
                  if (e.target.value.trim()) {
                    setPenandatangan({...penandatangan, ketua_panitia_nama: formatFullNameWithTitles(e.target.value)});
                  }
                }}
                className="rounded-xl" 
              />
              <p className="text-[10px] text-blue-600 mt-1">✓ Gelar akademik diformat otomatis sesuai ketentuan.</p>
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">NIP / NPK Ketua Panitia</label>
              <Input placeholder="NIP Ketua Panitia (atau - jika belum ada)" value={penandatangan.ketua_panitia_nip} onChange={e => setPenandatangan({...penandatangan, ketua_panitia_nip: e.target.value})} className="rounded-xl" />
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md rounded-2xl">
          ...
          <div className="space-y-4 pt-4">
            ...
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">Nama Guru & Gelar</label>
              <Input 
                ...
                onBlur={e => {
                  if (e.target.value.trim()) {
                    setGuruForm({...guruForm, nama: formatFullNameWithTitles(e.target.value)});
                  }
                }}
                className="rounded-xl" 
              />
            ...
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">NIP Guru</label>
              <Input placeholder="NIP Guru (atau - jika belum ada)" value={guruForm.nip} onChange={e => setGuruForm({...guruForm, nip: e.target.value})} className="rounded-xl" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">Kelas Diampu</label>
              <Input placeholder="Kelas (Contoh: Kelas 1 atau Kelas 1A)" value={guruForm.kelas} onChange={e => setGuruForm({...guruForm, kelas: e.target.value})} className="rounded-xl" />
            </div>
