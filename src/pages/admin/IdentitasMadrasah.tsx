...
import { formatFullNameWithTitles, formatOfficialSignatureName } from '@/utils/formatGelar';
...
export interface IdentitasMadrasahEmis {
  ...
  nama_resmi?: string; // Nama Resmi Madrasah / Satuan Pendidikan Resmi Pada Semua Dokumen
  nsm: string;
  npsn: string;
  ...
  // 7. Pengaturan Khusus Kop Surat (Dipisahkan dari Nama Resmi EMIS)
  use_custom_kop?: boolean;
  header_pengayom?: string;
  nama_yayasan_kop?: string;
  nama_kop?: string;
  sub_header_kop?: string;
  alamat_kop?: string;
  kontak_kop?: string;
  logo_kanan_url?: string;
  garis_style_kop?: 'double' | 'single' | 'thick' | 'accent' | 'none';
  font_size_nama_kop?: 'sm' | 'md' | 'lg' | 'xl';
...
const defaultEmisIdentitas: IdentitasMadrasahEmis = {
  ...
  nama_resmi: "MADRASAH IBTIDAIYAH MA'ARIF NU 2 SANGGREMAN",
  nsm: '111233020000',
  npsn: '60700000',
  ...
  // 7. Pengaturan Khusus Kop Surat
  use_custom_kop: false,
  header_pengayom: 'KEMENTERIAN AGAMA REPUBLIK INDONESIA',
  nama_yayasan_kop: 'YAYASAN PENDIDIKAN ISLAM',
  nama_kop: "MI MAARIF NU 2 SANGGREMAN",
  sub_header_kop: '',
  alamat_kop: '',
  kontak_kop: '',
  logo_kanan_url: '',
  garis_style_kop: 'double',
  font_size_nama_kop: 'lg',
};
...
const IdentitasMadrasah = () => {
  ...
  const [mediaTarget, setMediaTarget] = useState<'logo_url' | 'logo_kanan_url'>('logo_url');
    ...
    try {
      ...
      if (data?.value) {
        setIdentitas(prev => ({ 
          ...
          ...prev, 
          ...data.value,
          nama_resmi: data.value.nama_resmi || (activeMadrasah as any)?.nama_resmi || prev.nama_resmi || data.value.nama_madrasah
        }));
      } else {
        setIdentitas(prev => ({
          ...defaultEmisIdentitas,
          ...
          nama_resmi: (activeMadrasah as any).nama_resmi || prev.nama_resmi || activeMadrasah.nama_madrasah,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      try {
        localStorage.setItem('siakad_identitas_madrasah', JSON.stringify(payload));
      ...
      if (storageKey !== 'identitas_madrasah') {
        await supabase
          .from('site_settings')
          .upsert({ id: 'identitas_madrasah', value: payload, updated_at: new Date().toISOString() });
      }
      ...
      const officialSchoolName = identitas.nama_resmi?.trim() || identitas.nama_madrasah;
      await supabase
        .from('site_settings')
        .upsert({
          id: 'general',
          value: {
            ...existingGeneral,
            school_name: officialSchoolName,
            ...
            website: identitas.website || existingGeneral.website,
      ...
      await updateMadrasah(activeMadrasahId, {
        ...
        nama_resmi: officialSchoolName,
        nsm: identitas.nsm,
        npsn: identitas.npsn,
        nama_pimpinan: formatFullNameWithTitles(identitas.nama_pimpinan),
    ...
    <AdminLayout title="Profil Madrasah (EMIS Kemenag)">
      ...
      <div className="hidden print:block font-serif text-black p-6 space-y-6">
        ...
        <div className="mt-12 flex justify-between text-xs px-8">
          ...
          <div className="text-center">
            ...
            <p className="font-bold underline">{formatOfficialSignatureName(identitas.nama_pimpinan)}</p>
      ...
      <div className="print:hidden space-y-6">
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-6">
          <TabsList className="bg-slate-200/80 dark:bg-slate-800 p-1.5 rounded-2xl grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-1 h-auto">
            ...
            <TabsTrigger value="kop_surat" className="rounded-xl text-xs font-bold py-2.5 gap-1.5 data-[state=active]:bg-white data-[state=active]:text-emerald-800 shadow-sm">
              <Printer className="w-4 h-4 text-emerald-600" /> Kop Surat
            </TabsTrigger>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">
                      Nama Singkat / Nama Tampilan Madrasah <span className="text-red-500">*</span>
                    </label>
                    <Input
                      ...
                      className="rounded-xl text-xs font-semibold"
                    />
                    <p className="text-[10px] text-slate-500 mt-1">Nama ringkas atau sebutan operasional sehari-hari.</p>
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-emerald-900 mb-1.5 flex items-center justify-between">
                      <span className="flex items-center gap-1.5">
                        Nama Resmi Madrasah <span className="text-emerald-600 font-extrabold">*</span>
                      </span>
                      <Badge className="bg-emerald-100 text-emerald-800 border-emerald-300 text-[9px] px-1.5 py-0 font-bold">Identitas Resmi Dokumen</Badge>
                    </label>
                    <Input
                      placeholder="Contoh: MADRASAH IBTIDAIYAH MA'ARIF NU 2 SANGGREMAN"
                      value={identitas.nama_resmi ?? ''}
                      onChange={(e) => setIdentitas({ ...identitas, nama_resmi: e.target.value })}
                      className="rounded-xl text-xs font-bold text-emerald-950 bg-emerald-50/40 border-emerald-300 focus:border-emerald-500"
                    />
                    <p className="text-[10px] text-emerald-700 mt-1 font-medium">
                      Dicetak sebagai Nama Satuan Pendidikan resmi pada Rapor, LCKH, Kartu Ujian, Piagam, dan dokumen resmi.
                    </p>
                  </div>
                  ...
                  <div className="grid md:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-200">
                    <div>
                      ...
                      <Input
                        ...
                        onBlur={(e) => setIdentitas({ ...identitas, nama_pimpinan: formatFullNameWithTitles(e.target.value) })}
          ...
          {/* TAB 7: PENGATURAN KHUSUS KOP SURAT */}
          <TabsContent value="kop_surat">
            <Card className="border-0 shadow-xl rounded-3xl overflow-hidden">
              <CardHeader className="bg-gradient-to-r from-emerald-50 via-teal-50 to-emerald-100/50 border-b border-emerald-100">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    <CardTitle className="text-base font-bold flex items-center gap-2 text-slate-900">
                      <Printer className="w-5 h-5 text-emerald-600" />
                      Pengaturan Khusus Kop Surat Dokumen & Cetakan
                    </CardTitle>
                    <CardDescription className="text-xs text-slate-600 mt-1">
                      Kustomisasi tata letak, judul lembaga, sub-header, logo kanan/kiri, dan garis pembatas kop surat untuk seluruh cetakan ijazah, rapor, kwitansi & dokumen resmi.
                    </CardDescription>
                  </div>
                  <Badge className="bg-emerald-600 text-white font-bold text-[10px] uppercase px-3 py-1 shrink-0 self-start sm:self-auto">
                    Kop Surat Independen
                  </Badge>
                </div>
              </CardHeader>
              ...
              <CardContent className="p-6 space-y-6">
                ...
                <div className="p-4 rounded-2xl bg-amber-50/80 border border-amber-200/90 text-amber-900 space-y-2 text-xs">
                  <div className="flex items-center gap-2 font-bold text-amber-950">
                    <Sparkles className="w-4 h-4 text-amber-600" />
                    💡 Fitur Khusus: Pemisahan Nama Kop Surat dengan Nama Resmi EMIS
                  </div>
                  <p className="leading-relaxed text-amber-800">
                    Nama Resmi Madrasah di EMIS (misal: <strong className="underline">{identitas.nama_madrasah}</strong>) dapat berbeda dari teks yang dicetak pada Judul Kop Surat. Menggunakan pengaturan di bawah ini memungkinkan Anda memformat susunan Kop Surat sesuai standar instansi/dinas tanpa mengubah data resmi EMIS Kemenag.
                  </p>
                </div>

                {/* Toggle Mode Kustom */}
                <div className="flex items-center justify-between p-4 rounded-2xl bg-emerald-50/60 border border-emerald-200/80">
                  <div className="space-y-0.5">
                    <label className="text-xs font-bold text-slate-900 flex items-center gap-2">
                      Status Pengaturan Kop Surat
                    </label>
                    <p className="text-[11px] text-slate-500">
                      {identitas.use_custom_kop 
                        ? '✨ Menggunakan Pengaturan Kustom Kop Surat Khusus (Aktif)' 
                        : '⚡ Menggunakan Format Otomatis dari Identitas Madrasah EMIS'}
                    </p>
                  </div>
                  <Button
                    type="button"
                    onClick={() => setIdentitas({ ...identitas, use_custom_kop: !identitas.use_custom_kop })}
                    className={identitas.use_custom_kop ? 'bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl px-4' : 'bg-slate-200 hover:bg-slate-300 text-slate-800 font-bold text-xs rounded-xl px-4'}
                  >
                    {identitas.use_custom_kop ? 'Mode Kustom (Aktif)' : 'Aktifkan Mode Kustom'}
                  </Button>
                </div>

                {/* Form Fields Kop Surat */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="md:col-span-2 space-y-1.5">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-bold text-slate-800">
                        Baris Instansi Atas / Header Pengayom
                      </label>
                      <span className="text-[10px] text-slate-400">Contoh: KEMENTERIAN AGAMA REPUBLIK INDONESIA</span>
                    </div>
                    <Input 
                      value={identitas.header_pengayom ?? 'KEMENTERIAN AGAMA REPUBLIK INDONESIA'}
                      onChange={(e) => setIdentitas({ ...identitas, header_pengayom: e.target.value })}
                      placeholder="Contoh: KEMENTERIAN AGAMA REPUBLIK INDONESIA"
                      className="rounded-xl h-10 font-semibold text-slate-900 border-slate-300"
                    />
                  </div>

                  {/* Nama Yayasan di Kop Surat */}
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-bold text-slate-800">
                        Nama Yayasan / Penyelenggara di Kop Surat
                      </label>
                      <span className="text-[10px] text-slate-400">Opsional</span>
                    </div>
                    <Input 
                      value={identitas.nama_yayasan_kop ?? identitas.nama_yayasan ?? ''}
                      onChange={(e) => setIdentitas({ ...identitas, nama_yayasan_kop: e.target.value })}
                      placeholder="Contoh: YAYASAN PENDIDIKAN ISLAM AL-IKHLAS"
                      className="rounded-xl h-10 border-slate-300"
                    />
                  </div>

                  {/* Nama Kop Surat Utama (Membedakan Nama Kop Surat vs EMIS) */}
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-bold text-emerald-900 flex items-center gap-1">
                        <Pencil className="w-3.5 h-3.5 text-emerald-600" />
                        Nama Kop Surat Utama (Membedakan Nama Kop vs EMIS) <span className="text-red-500">*</span>
                      </label>
                      {(identitas.nama_resmi || identitas.nama_madrasah) && (
                        <button
                          type="button"
                          onClick={() => setIdentitas({ ...identitas, nama_kop: identitas.nama_resmi || identitas.nama_madrasah })}
                          className="text-[10px] text-emerald-700 hover:underline font-bold"
                        >
                          Samakan dg Nama Resmi
                        </button>
                      )}
                    </div>
                    <Input 
                      value={identitas.nama_kop ?? identitas.nama_resmi ?? identitas.nama_madrasah ?? ''}
                      onChange={(e) => setIdentitas({ ...identitas, nama_kop: e.target.value })}
                      placeholder="Contoh: MADRASAH IBTIDAIYAH NEGERI 1 KOTA BANDUNG"
                      className="rounded-xl h-10 font-bold text-emerald-950 border-emerald-300 bg-emerald-50/30"
                    />
                    <p className="text-[10px] text-slate-500">
                      * Teks ini yang akan dicetak sebagai judul lembaga utama pada bagian tengah Kop Surat.
                    </p>
                  </div>
                  ...
                  <div className="md:col-span-2 space-y-1.5">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-bold text-slate-800">
                        Sub-Header / Keterangan Legalitas & Akreditasi
                      </label>
                      <span className="text-[10px] text-slate-400">NSM, NPSN, Akreditasi</span>
                    </div>
                    <Input 
                      value={identitas.sub_header_kop ?? `NSM: ${identitas.nsm || '-'} | NPSN: ${identitas.npsn || '-'} | AKREDITASI ${identitas.akreditasi || 'A'}`}
                      onChange={(e) => setIdentitas({ ...identitas, sub_header_kop: e.target.value })}
                      placeholder="Contoh: TERAKREDITASI A | NSM: 1112330... | NPSN: 60700..."
                      className="rounded-xl h-10 border-slate-300 font-medium"
                    />
                  </div>

                  {/* Alamat Khusus Kop Surat */}
                  <div className="md:col-span-2 space-y-1.5">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-bold text-slate-800">
                        Alamat Lengkap Baris Kop Surat
                      </label>
                      <button
                        type="button"
                        onClick={() => {
                          const detail = [
                            identitas.alamat,
                            identitas.rt_rw,
                            identitas.dusun && identitas.dusun !== identitas.desa ? identitas.dusun : '',
                            identitas.desa,
                            identitas.kecamatan ? `Kec. ${identitas.kecamatan}` : '',
                            identitas.kabupaten ? (identitas.kabupaten.toLowerCase().startsWith('kab') ? identitas.kabupaten : `Kab. ${identitas.kabupaten}`) : '',
                            identitas.provinsi,
                            identitas.kode_pos
                          ].filter(Boolean).join(', ');
                          setIdentitas({ ...identitas, alamat_kop: detail });
                        }}
                        className="text-[10px] text-emerald-700 hover:underline font-bold"
                      >
                        🔄 Sinkronkan Alamat dari Profil
                      </button>
                    </div>
                    <Input 
                      value={identitas.alamat_kop || ''}
                      onChange={(e) => setIdentitas({ ...identitas, alamat_kop: e.target.value })}
                      placeholder={`Otomatis: ${identitas.alamat || 'Jl. Raya Sanggreman No. 12'}, Kec. ${identitas.kecamatan || 'Rawalo'}, Kab. ${identitas.kabupaten || 'Banyumas'}`}
                      className="rounded-xl h-10 border-slate-300"
                    />
                    <p className="text-[10px] text-slate-500">
                      * Kosongkan agar selalu mengikuti perubahan data alamat dari Profil Madrasah secara otomatis.
                    </p>
                  </div>

                  {/* Kontak Khusus Kop Surat */}
                  <div className="md:col-span-2 space-y-1.5">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-bold text-slate-800">
                        Baris Kontak & Media Informasi Kop Surat
                      </label>
                      <button
                        type="button"
                        onClick={() => {
                          const contactParts = [
                            identitas.telepon ? `Telp: ${identitas.telepon}` : '',
                            identitas.email ? `Email: ${identitas.email}` : '',
                            identitas.website ? `Web: ${identitas.website.replace(/^https?:\/\//i, '')}` : ''
                          ].filter(Boolean).join(' | ');
                          setIdentitas({ ...identitas, kontak_kop: contactParts });
                        }}
                        className="text-[10px] text-emerald-700 hover:underline font-bold"
                      >
                        🔄 Sinkronkan Kontak dari Profil
                      </button>
                    </div>
                    <Input 
                      value={identitas.kontak_kop || ''}
                      onChange={(e) => setIdentitas({ ...identitas, kontak_kop: e.target.value })}
                      placeholder={`Otomatis: Telp: ${identitas.telepon || '-'} | Email: ${identitas.email || '-'} | Web: ${(identitas.website || '-').replace(/^https?:\/\//i, '')}`}
                      className="rounded-xl h-10 border-slate-300"
                    />
                    <p className="text-[10px] text-slate-500">
                      * Kosongkan agar otomatis mengalirkan data telepon, email, dan website resmi dari Profil Madrasah.
                    </p>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-800 flex items-center justify-between">
                      <span>Logo Sisi Kanan Kop Surat (Opsional)</span>
                      <span className="text-[10px] text-slate-400">Logo Kemenag / Pemda / Yayasan</span>
                    </label>
                    <div className="flex items-center gap-3">
                      <div className="w-14 h-14 rounded-xl border bg-slate-50 flex items-center justify-center shrink-0 overflow-hidden">
                        {identitas.logo_kanan_url ? (
                          <img src={identitas.logo_kanan_url} alt="Logo Kanan" className="w-full h-full object-contain p-1" />
                        ) : (
                          <span className="text-[8px] text-slate-400 font-bold">KOSONG</span>
                        )}
                      </div>
                      <div className="flex-1 space-y-1">
                        <Input 
                          value={identitas.logo_kanan_url ?? ''}
                          onChange={(e) => setIdentitas({ ...identitas, logo_kanan_url: e.target.value })}
                          placeholder="URL Logo Kanan atau Upload dari Galeri"
                        ...
                        <div className="flex gap-2">
                          <Button
                            ...
                            size="sm"
                            onClick={() => {
                              setMediaTarget('logo_kanan_url');
                              setMediaModalOpen(true);
                            }}
                            className="rounded-lg text-[10px] h-7 gap-1"
                          >
                            <ImageIcon className="w-3 h-3 text-emerald-600" /> Dari Galeri
                          </Button>
                          {identitas.logo_kanan_url && (
                            <Button
                              type="button"
                              variant="ghost"
                              size="sm"
                              onClick={() => setIdentitas({ ...identitas, logo_kanan_url: '' })}
                              className="rounded-lg text-[10px] h-7 text-red-600 hover:bg-red-50"
                            >
                              Hapus
                            </Button>
                          )}
                  ...
                  {/* Style Garis Pembatas & Font Size */}
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-bold text-slate-800 mb-1">
                        Gaya Garis Bawah Kop
                      </label>
                      <select
                        value={identitas.garis_style_kop || 'double'}
                        onChange={(e) => setIdentitas({ ...identitas, garis_style_kop: e.target.value as any })}
                        className="w-full h-10 rounded-xl border border-slate-300 bg-white px-3 text-xs font-semibold text-slate-800"
                      >
                        <option value="double">══ Garis Ganda Dinas (Double)</option>
                        <option value="single">── Garis Tunggal (Single)</option>
                        <option value="thick">━━ Garis Tebal (Thick)</option>
                        <option value="accent">✨ Garis Emerald Accent</option>
                        <option value="none">❌ Tanpa Garis Bawah</option>
                      </select>
                    </div>

                    <div>
                      <label className="block text-xs font-bold text-slate-800 mb-1">
                        Ukuran Font Nama Kop
                      </label>
                      <select
                        value={identitas.font_size_nama_kop || 'lg'}
                        onChange={(e) => setIdentitas({ ...identitas, font_size_nama_kop: e.target.value as any })}
                        className="w-full h-10 rounded-xl border border-slate-300 bg-white px-3 text-xs font-semibold text-slate-800"
                      >
                        <option value="sm">Kecil (13pt)</option>
                        <option value="md">Sedang (15pt)</option>
                        <option value="lg">Standar (17pt)</option>
                        <option value="xl">Besar (19pt)</option>
                      </select>
                    </div>
                  </div>
                ...
                <div className="mt-8 pt-6 border-t border-slate-200">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider flex items-center gap-2">
                      ...
                      Pratinjau Langsung Hasil Kop Surat Dokumen (Live Preview)
                    </h4>
                  ...
                  <div className="p-6 bg-white border border-slate-300 rounded-2xl shadow-sm overflow-x-auto">
                    <KopSurat 
                      customData={{
                        use_custom_kop: true,
                        header_pengayom: identitas.header_pengayom,
                        nama_yayasan: identitas.nama_yayasan_kop || identitas.nama_yayasan,
                        nama_kop: identitas.nama_kop || identitas.nama_madrasah,
                        sub_header: identitas.sub_header_kop,
                        alamat: identitas.alamat_kop,
                        kontak: identitas.kontak_kop,
                        telepon: identitas.telepon,
                        email: identitas.email,
                        website: identitas.website,
                        logo_url: identitas.logo_url,
                        logo_kanan_url: identitas.logo_kanan_url,
                        garis_style: identitas.garis_style_kop || 'double',
                        font_size_nama: identitas.font_size_nama_kop || 'lg',
                      }}
                    />
                    <div className="text-center text-[10px] text-slate-400 italic pt-2 border-t border-dashed border-slate-200">
                      — Batas Atas Isi Surat / Dokumen Resmi —
                    </div>
                  </div>
                </div>
              </CardContent>
        ...
        <MediaLibraryModal
          ...
          onSelectImage={(url) => {
            if (mediaTarget === 'logo_kanan_url') {
              setIdentitas(prev => ({ ...prev, logo_kanan_url: url }));
              showSuccess('Logo Kanan Kop Surat diperbarui!');
            } else {
              setIdentitas(prev => ({ ...prev, logo_url: url }));
              showSuccess('Logo diperbarui dari Galeri Foto!');
            }
          }}
          title={mediaTarget === 'logo_kanan_url' ? "Pilih Logo Kanan Kop Surat" : "Pilih Logo dari Galeri Foto"}
        />
