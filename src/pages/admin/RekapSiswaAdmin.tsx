...
import { loadPersistedClasses, loadPersistedStudents, DEFAULT_ROMBEL_CLASSES } from '@/utils/rombelPersistence';
...
const RekapSiswaAdmin = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const persistedClasses = await loadPersistedClasses(classesKey);
      if (persistedClasses.length > 0) {
        loadedClasses = persistedClasses;
      }

      const persistedStudents = await loadPersistedStudents(studentsKey);
      if (persistedStudents.length > 0) {
        loadedStudents = persistedStudents;
      }
  ...
  const { effectiveStudents, effectiveClasses, isArchivedView, archiveInfo } = useMemo(() => {
    ...
    const sourceClasses = (Array.isArray(classes) && classes.length > 0) ? classes : DEFAULT_ROMBEL_CLASSES;
    ...
    const sortedEffectiveClasses = Array.from(classMap.values()).sort((a, b) => {
