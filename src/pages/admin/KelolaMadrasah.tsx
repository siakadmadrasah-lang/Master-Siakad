...
const KelolaMadrasah = () => {
  ...
  const handleOpenEdit = (m: MadrasahItem) => {
    ...
    setFormData({
      ...
      logo_url: m.logo_url || '',
      custom_password: (m as any).custom_password || '',
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const officialName = formData.nama_resmi.trim() || formData.nama_madrasah.trim();
      if (editingMadrasah) {
        const ok = await updateMadrasah(editingMadrasah.id, {
          ...formData,
          nama_resmi: officialName,
          email: formData.email.trim().toLowerCase(),
        });
        if (ok) {
          try {
            const identitasKey = getScopedKey('identitas_madrasah', editingMadrasah.id);
            const { data: identData } = await supabase.from('site_settings').select('value').eq('id', identitasKey).maybeSingle();
            const existingIdent = identData?.value || {};
            await supabase.from('site_settings').upsert({
              ...
              value: {
                ...
                nip_pimpinan: formData.nip_pimpinan,
            ...
            if (editingMadrasah.id === activeMadrasahId) {
              const { data: genData } = await supabase.from('site_settings').select('value').eq('id', 'general').maybeSingle();
              const existingGen = genData?.value || {};
              await supabase.from('site_settings').upsert({
                id: 'general',
                value: {
                  ...existingGen,
                  school_name: officialName,
                },
                updated_at: new Date().toISOString()
              });
            ...
            console.warn('Sync to identitas_madrasah:', syncErr);
          }
          setDialogOpen(false);
        }
      } else {
                ...
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      {m.nama_resmi && m.nama_resmi !== m.nama_madrasah && (
                        <p className="text-[11px] font-semibold text-emerald-800 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200 mt-1 inline-block">
                          <span className="text-[10px] text-emerald-600 font-bold uppercase tracking-wider mr-1">Resmi:</span>{m.nama_resmi}
                        </p>
                      )}
        ...
        <DialogContent className="sm:max-w-xl rounded-3xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSave} className="space-y-4 pt-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Nama Madrasah / Tampilan Singkat *</label>
                <Input
                  required
                ...
                />
                <p className="text-[10px] text-slate-500 mt-1">Nama sebutan madrasah pada sidebar dan menu.</p>
              </div>

              <div>
                <label className="block text-xs font-bold text-emerald-900 mb-1 flex items-center justify-between">
                  <span>Nama Resmi Madrasah *</span>
                  <Badge className="bg-emerald-100 text-emerald-800 border-emerald-300 text-[9px] px-1 py-0 font-bold">Identitas Dokumen</Badge>
                </label>
                <Input
                  placeholder="Contoh: MADRASAH IBTIDAIYAH AL-IKHLAS"
                  value={formData.nama_resmi}
                  onChange={(e) => setFormData({ ...formData, nama_resmi: e.target.value })}
                  className="rounded-xl text-xs font-bold text-emerald-950 bg-emerald-50/40 border-emerald-300"
                />
                <p className="text-[10px] text-emerald-700 mt-1 font-medium">
                  Nama Satuan Pendidikan resmi di Rapor, LCKH, Kartu Ujian, Piagam, dan seluruh dokumen.
                </p>
              </div>
