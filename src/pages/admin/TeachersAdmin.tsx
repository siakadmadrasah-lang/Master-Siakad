...
import { 
  Users, User, Briefcase, Plus, Search, Pencil, Trash2, Printer, Download, RefreshCw, 
...
import { downloadExcelWithDropdowns, GTK_DROPDOWN_OPTIONS } from '@/utils/excelTemplateHelper';
import { normalizeDateToYMD, cleanNipField, executeAutoSyncTeachers } from '@/utils/gsheetParserHelper';
import { formatFullNameWithTitles, normalizeAcademicTitle, formatTeacherDisplayName } from '@/utils/formatGelar';
...
export interface Teacher {
  ...
  nrg?: string; // NRG (Nomor Registrasi Guru)
  ...
  alamat_rumah?: string; // Alamat Rumah / Domisili Lengkap
...
const sampleTeachers: Teacher[] = [
  {
    ...
    alamat_rumah: 'Jl. Raya Kebasen No. 12, RT 02/03, Banyumas',
  ...
  },
  {
    ...
    alamat_rumah: 'Desa Mandirancan RT 01/RW 04, Kebasen',
  ...
  },
  {
    ...
    alamat_rumah: 'Jl. Ahmad Yani No. 45, Purwokerto Timur',
  ...
  },
  {
    ...
    alamat_rumah: 'Jl. Diponegoro RT 03/01, Sampang, Cilacap',
  ...
  },
  {
    ...
    alamat_rumah: 'Perumahan Griya Satria Blok C-10, Banyumas',
...
const TeachersAdmin = () => {
  ...
  const checkBackgroundAutoSync = useCallback(async (currentList: Teacher[]) => {
    const isAuto = localStorage.getItem('siakad_gsheet_autosync_teachers') === 'true';
    const sheetUrl = localStorage.getItem('siakad_gsheet_url_teachers');
    const sheetTab = localStorage.getItem('siakad_gsheet_sheetname_teachers') || 'Data_GTK';

    if (isAuto && sheetUrl && currentList && currentList.length > 0) {
      try {
        setIsAutoSyncing(true);
        const result = await executeAutoSyncTeachers(sheetUrl, sheetTab, currentList);
        if (result.success && result.data && (result.updatedCount > 0 || result.newCount > 0)) {
          setTeachers(result.data);
          saveTeachersToDb(result.data);
          const nowStr = new Date().toLocaleString('id-ID');
          localStorage.setItem('siakad_gsheet_last_sync_teachers', nowStr);
          showSuccess(`⚡ Auto-Sync Google Sheets: ${result.updatedCount} data diperbarui, ${result.newCount} data baru.`);
        }
      } catch (err) {
        ...
        if (cached) {
          try {
            ...
            if (Array.isArray(parsed)) {
              setTeachers(parsed);
              checkBackgroundAutoSync(parsed);
              return;
            }
          } catch (e) {
  ...
  const handleOpenEdit = (teacher: Teacher) => {
    ...
    setFormData({
      ...
      email: teacher.email || '',
      alamat_rumah: teacher.alamat_rumah || '',
      ...
      tmt_madrasah: teacher.tmt_madrasah || '',
    });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const normalizedData = {
        ...formData,
        nama: formatFullNameWithTitles(formData.nama),
        gelar: formData.gelar ? normalizeAcademicTitle(formData.gelar) : ''
      };

      let updatedList: Teacher[] = [];
      if (editingTeacher) {
        updatedList = teachers.map(t => t.id === editingTeacher.id ? { ...t, ...normalizedData } : t);
  ...
  const filteredTeachers = useMemo(() => {
    return teachers.filter(t => {
      const matchesSearch = 
        ...
        (t.nrg || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
        (t.tempat_lahir || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
        (t.tanggal_lahir || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
        (t.alamat_rumah || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
        (t.nuptk || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
        (t.peg_id || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  // Download Excel Template for GTK (with Dropdown Validation)
  const handleDownloadTemplate = async (withExistingData: boolean = false) => {
    try {
      const dataToExport = withExistingData && teachers.length > 0 ? teachers : undefined;
      const fileName = withExistingData && teachers.length > 0
        ? `Data_GTK_Lengkap_${activeMadrasah.nama_madrasah || 'Madrasah'}_${new Date().toISOString().slice(0, 10)}.xlsx`
        : `Template_Data_Guru_GTK_${activeMadrasah.nama_madrasah || 'Madrasah'}.xlsx`;
      ...
      if (withExistingData && teachers.length > 0) {
        showSuccess(`Berhasil mengunduh ${teachers.length} data GTK ke dalam format Spreadsheet dengan validasi Dropdown aktif!`);
      } else {
        showSuccess('Template Excel Data Guru & GTK (dengan validasi Dropdown & Alamat Lengkap) berhasil diunduh!');
      }
    } catch (err: any) {
  ...
  // Export Teachers to Real Excel (.xlsx) with Dropdown & Plain Text Protection
  const handleExportExcel = async () => {
    ...
    try {
      await downloadExcelWithDropdowns(
        `Data_Guru_GTK_${activeMadrasah.nama_madrasah || 'Madrasah'}`,
        'Data_GTK',
        filteredTeachers
      );
      showSuccess('Data Guru berhasil diexport ke Excel (.xlsx) dengan proteksi teks angka 16-digit & dropdown!');
    } catch {
      // Fallback
      const exportRows = filteredTeachers.map((t, idx) => ({
        ...
        'NPK Kemenag': t.npk ? `'${t.npk}` : '',
        'Peg ID Simpatika': t.peg_id ? `'${t.peg_id}` : '',
        'NRG': t.nrg ? `'${t.nrg}` : '',
        'Jabatan': t.jabatan || '',
        'Status Kepegawaian': t.status_kepegawaian || '',
        'Jenis Kelamin': t.gender || '',
        'Alamat Rumah': t.alamat_rumah || '',
        'No WhatsApp / Telepon': t.telepon ? `'${t.telepon}` : '',
        'Email': t.email || '',
        'Pendidikan Terakhir': t.pendidikan || '',
        'Status Sertifikasi': t.sertifikasi || '',
        'No. Sertifikat Pendidik': t.no_sertifikat_pendidik ? `'${t.no_sertifikat_pendidik}` : '',
  ...
  const handleImportExcel = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const workbook = XLSX.read(data, { type: 'array', cellDates: true, cellNF: true, cellText: true, raw: false });
        ...
        // Helper to normalize and convert 16-digit numbers or scientific notations without losing trailing digits
          ...
          if (/^[0-9]+(\.[0-9]+)?[eE][\+\-]?[0-9]+$/.test(str)) {
            try {
              ...
              if (exp > 0) {
                ...
                const frac = parts[1] || '';
                if (frac.length <= exp) {
                  str = whole + frac + '0'.repeat(exp - frac.length);
                } else {
                  str = whole + frac.substring(0, exp);
                }
              }
            } catch {
        ...
        formattedRows.forEach((row, idx) => {
          ...
          const getVal = (aliases: string[], isNumericField = false): string => {
            ...
            for (const key of allKeys) {
              ...
              if (cleanAliases.includes(cleanKey)) {
                ...
                let chosenVal = '';
                if (isNumericField) {
                  const formStr = formVal !== undefined && formVal !== null ? String(formVal).trim() : '';
                  const rawStr = rawVal !== undefined && rawVal !== null ? String(rawVal).trim() : '';
                  if (formStr !== '' && !/^[0-9]+(\.[0-9]+)?[eE]/.test(formStr)) {
                    chosenVal = cleanNumberField(formStr);
                  } else if (rawStr !== '' && !/^[0-9]+(\.[0-9]+)?[eE]/.test(rawStr)) {
                    chosenVal = cleanNumberField(rawStr);
                  } else if (formStr !== '') {
                    chosenVal = cleanNumberField(formStr);
                  } else if (rawStr !== '') {
                    chosenVal = cleanNumberField(rawStr);
                  ...
                  if (formVal !== undefined && formVal !== null && String(formVal).trim() !== '') {
                    chosenVal = String(formVal).trim();
                  } else if (rawVal !== undefined && rawVal !== null && String(rawVal).trim() !== '') {
                    chosenVal = String(rawVal).trim();
                  }
                }
          ...
          const nip = cleanNipField(getVal(['NIP', 'nip', 'NIP / NPK', 'NIP/NPK', 'Nomor Induk Pegawai', 'No. NIP'], true));
          ...
          const nrg = getVal(['NRG', 'nrg', 'No NRG', 'No. NRG', 'Nomor Registrasi Guru', 'Nomor NRG', 'nrg guru', 'no_nrg', 'No. Registrasi Guru', 'No Registrasi Guru'], true);
          ...
          const alamat_rumah = getVal(['Alamat Rumah', 'Alamat', 'alamat_rumah', 'Alamat Domisili', 'Alamat Lengkap', 'Alamat Tinggal', 'Domisili']);
          ...
          const tempat_lahir = getVal(['Tempat Lahir', 'tempat_lahir', 'Tempat Lahir Guru', 'Tempat', 'Kota Lahir', 'tempat_tgl_lahir', 'tempat']);
          const rawTanggalLahir = getVal(['Tanggal Lahir (YYYY-MM-DD)', 'Tanggal Lahir', 'tanggal_lahir', 'Tgl Lahir', 'Tgl. Lahir', 'tgl_lahir', 'Tgl Lahir Guru', 'Tanggal']);
          const tanggal_lahir = normalizeDateToYMD(rawTanggalLahir);
          const rawTmtPendidik = getVal(['TMT Pendidik', 'TMT Pendidik (YYYY-MM-DD)', 'TMT', 'tmt_pendidik', 'TMT Guru', 'TMT Awal Pendidik', 'TMT Awal']);
          const tmt_pendidik = normalizeDateToYMD(rawTmtPendidik);
          const rawTmtMadrasah = getVal(['TMT Madrasah', 'TMT Madrasah (YYYY-MM-DD)', 'TMT Satminkal', 'tmt_madrasah', 'TMT Sekolah', 'TMT Tugas di Madrasah', 'TMT Tugas Satminkal']);
      ...
      <div className="print:hidden space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-emerald-900 via-teal-900 to-slate-900 text-white p-6 rounded-3xl shadow-xl">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              onClick={() => setGsheetSyncOpen(true)}
              variant="outline"
              size="sm"
              className="bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-100 border-emerald-400/40 rounded-2xl gap-2 text-xs font-bold shadow-sm"
            >
              <FileSpreadsheet className="w-3.5 h-3.5 text-emerald-300" />
              <span>Live Sync Google Sheet</span>
              {isAutoSyncing ? (
                <RefreshCw className="w-3 h-3 text-emerald-300 animate-spin" />
              ) : (
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" title="Google Sheets Auto-Sync Ready"></span>
              )}
            </Button>
            <Button
                          ...
                          <div>
                            ...
                            {(teacher.tempat_lahir || teacher.tanggal_lahir) && (
                              <div className="text-[10px] text-slate-500 font-medium mt-0.5">
                                TTL: {teacher.tempat_lahir || '-'}{teacher.tanggal_lahir ? `, ${teacher.tanggal_lahir}` : ''}
                              </div>
                            )}
                            {(teacher.tmt_pendidik || teacher.tmt_madrasah) && (
                              <div className="text-[10px] text-slate-500 font-medium mt-0.5 flex flex-wrap items-center gap-2">
                                {teacher.tmt_pendidik && <span className="text-slate-600">TMT Pendidik: <strong className="text-slate-800">{teacher.tmt_pendidik}</strong></span>}
                                {teacher.tmt_madrasah && <span className="text-slate-600">TMT Madrasah: <strong className="text-slate-800">{teacher.tmt_madrasah}</strong></span>}
                              </div>
                            )}
                            {(teacher.nrg || teacher.npk || teacher.peg_id || teacher.telepon) && (
                              <div className="text-[10px] text-slate-400 font-mono flex flex-wrap items-center gap-2 mt-0.5">
                                {teacher.nrg && <span className="text-emerald-800 font-bold bg-emerald-50 px-1 rounded border border-emerald-200">NRG: {teacher.nrg}</span>}
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-5 pt-2">
              ...
              <div className="bg-slate-50/80 p-3.5 rounded-2xl border border-slate-200/80 space-y-3">
                <div className="flex items-center gap-2 pb-1 border-b border-slate-200 text-xs font-black text-slate-800 uppercase tracking-wider">
                  <User className="w-3.5 h-3.5 text-emerald-600" />
                  1. Identitas Pribadi & Kependudukan
                </div>
                <div className="grid md:grid-cols-2 gap-3.5">
                    ...
                    <Input
                      ...
                      onBlur={(e) => {
                        if (e.target.value.trim()) {
                          setFormData(prev => ({ ...prev, nama: formatFullNameWithTitles(e.target.value) }));
                        }
                      }}
                      placeholder="Contoh: Ahmad Syafii, S.Pd.I"
                      className="rounded-xl text-xs font-bold bg-white"
                      required
                    />
                    <p className="text-[10px] text-emerald-700 font-medium mt-1">
                      Format gelar &amp; EYD otomatis (misal: S. PD.I → S.Pd.I).
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onBlur={(e) => {
                        if (e.target.value.trim()) {
                          setFormData(prev => ({ ...prev, gelar: normalizeAcademicTitle(e.target.value) }));
                        }
                      }}
                    ...
                    {formData.nama && (
                      <p className="text-[10px] text-slate-500 font-medium mt-1 truncate">
                        Tampilan: <strong className="text-slate-800">{formatTeacherDisplayName(formData)}</strong>
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      className="rounded-xl text-xs font-mono font-bold bg-white"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      placeholder="Kota / Kabupaten Lahir (misal: Banyumas)"
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-xs font-bold text-slate-700 mb-1">
                      Alamat Rumah / Domisili Lengkap
                    </label>
                    <Input
                      value={formData.alamat_rumah || ''}
                      onChange={(e) => setFormData({ ...formData, alamat_rumah: e.target.value })}
                      placeholder="Contoh: Jl. Raya Timur No. 12, RT 02/RW 03, Desa Karanganyar, Banyumas"
                      className="rounded-xl text-xs font-semibold bg-white"
                    />
                  </div>
                </div>
              ...
              {/* SECTION 2: KEPEGAWAIAN & KEMENAG */}
              <div className="bg-emerald-50/50 p-3.5 rounded-2xl border border-emerald-200/80 space-y-3">
                <div className="flex items-center gap-2 pb-1 border-b border-emerald-200 text-xs font-black text-emerald-900 uppercase tracking-wider">
                  <ShieldCheck className="w-3.5 h-3.5 text-emerald-600" />
                  2. Nomor Registrasi & Kepegawaian Kemenag
                </div>
                <div className="grid md:grid-cols-2 gap-3.5">
                  <div>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">NPK Kemenag (12 Digit)</label>
                    <Input
                      value={formData.npk || ''}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-emerald-900 mb-1 flex items-center justify-between">
                      <span>NRG (Nomor Registrasi Guru)</span>
                      <span className="text-[10px] font-bold text-emerald-600 bg-emerald-100 px-1.5 py-0.2 rounded">Simpatika / Kemenag</span>
                    </label>
                    <Input
                      value={formData.nrg || ''}
                      onChange={(e) => setFormData({ ...formData, nrg: e.target.value })}
                      placeholder="Nomor Registrasi Guru (NRG)"
                      className="rounded-xl text-xs font-mono bg-white border-emerald-300 focus:border-emerald-500 font-bold text-emerald-900"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">TMT Pendidik (Tgl Mulai Tugas Awal)</label>
                    <Input
                      type="date"
                      value={formData.tmt_pendidik || ''}
                      onChange={(e) => setFormData({ ...formData, tmt_pendidik: e.target.value })}
                      className="rounded-xl text-xs font-semibold bg-white"
                    />
                  </div>

                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">TMT Madrasah (Tgl Mulai di Madrasah Ini)</label>
                    <Input
                      type="date"
                      value={formData.tmt_madrasah || ''}
                      onChange={(e) => setFormData({ ...formData, tmt_madrasah: e.target.value })}
                      className="rounded-xl text-xs font-semibold bg-white"
                    />
                  </div>
                </div>
              ...
              {/* SECTION 3: PENDIDIKAN & SERTIFIKASI */}
              <div className="bg-slate-50/80 p-3.5 rounded-2xl border border-slate-200/80 space-y-3">
                <div className="flex items-center gap-2 pb-1 border-b border-slate-200 text-xs font-black text-slate-800 uppercase tracking-wider">
                ...
                <div className="grid md:grid-cols-2 gap-3.5">
                  <div>
                    ...
                    <datalist id="list-pendidikan">
                      {GTK_DROPDOWN_OPTIONS.pendidikan.map(p => (
                  ...
                  <div className="md:col-span-2">
                    ...
                    <Input
                      ...
                      placeholder="12 Digit No. Sertifikat Pendidik (jika sudah sertifikasi)"
                      className="rounded-xl text-xs font-mono bg-white"
                    />
                  </div>
                </div>
              ...
              {/* SECTION 4: TUGAS, KONTAK & AKTIF */}
              <div className="bg-slate-50/80 p-3.5 rounded-2xl border border-slate-200/80 space-y-3">
                <div className="flex items-center gap-2 pb-1 border-b border-slate-200 text-xs font-black text-slate-800 uppercase tracking-wider">
                  <Briefcase className="w-3.5 h-3.5 text-emerald-600" />
                  4. Tugas, Mengajar & Kontak
                </div>
                <div className="grid md:grid-cols-2 gap-3.5">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1 flex items-center justify-between">
                      <span>Jabatan Guru / Tugas</span>
                      <span className="text-[10px] text-slate-400 font-normal">Pilih / Ketik</span>
                    </label>
                    <Input
                      list="list-jabatan"
                    ...
                    <datalist id="list-jabatan">
                      {GTK_DROPDOWN_OPTIONS.jabatan.map(j => (
                  ...
                  <div>
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="Aktif">Aktif Mengajar</SelectItem>
                        <SelectItem value="Cuti">Cuti</SelectItem>
                        <SelectItem value="Tugas Belajar">Tugas Belajar</SelectItem>
                        <SelectItem value="Pindah Tugas">Pindah Tugas</SelectItem>
                        <SelectItem value="Non-Aktif">Non-Aktif</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  <div className="md:col-span-2 pt-2 border-t border-slate-200/60">
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">Foto Profil Guru / GTK</label>
                    <div className="flex items-center gap-3">
                      {formData.foto_url ? (
                        <img src={formatImageUrl(formData.foto_url)} alt="Foto" className="w-14 h-14 rounded-2xl object-cover border-2 border-emerald-400 shadow-sm" />
                      ) : (
                        <div className="w-14 h-14 rounded-2xl bg-slate-200/80 border border-slate-300 flex items-center justify-center text-slate-400">
                          <ImageIcon className="w-6 h-6" />
                        </div>
                      )}
                      <div className="flex flex-wrap gap-2">
                        <label className="cursor-pointer">
                          <Button type="button" size="sm" variant="outline" className="rounded-xl text-xs font-bold bg-white hover:bg-slate-50" asChild disabled={uploading}>
                            <span><Upload className="w-3.5 h-3.5 mr-1 text-emerald-600" /> {uploading ? 'Mengunggah...' : 'Upload Foto'}</span>
                          </Button>
                        ...
                        <Button
                          ...
                          className="rounded-xl text-xs font-bold bg-white hover:bg-slate-50"
                        >
                          Pilih dari Galeri
                        </Button>
                        {formData.foto_url && (
                          <Button
                            ...
                            className="rounded-xl text-xs font-bold text-red-500 hover:text-red-700 hover:bg-red-50"
                          >
                            Hapus Foto
                          </Button>
                        )}
            ...
            {importStep === 'upload' ? (
              <>
                ...
                <div className="space-y-4 py-2">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-3.5 bg-slate-50 border border-slate-200 rounded-2xl">
                    <div>
                      <div className="text-xs font-bold text-slate-800">Unduh Format Template Spreadsheet</div>
                      <div className="text-[10px] text-slate-500">Dilengkapi validasi Dropdown bawaan dan format identik modul Guru</div>
                    </div>
                    <div className="flex flex-wrap items-center gap-1.5 shrink-0">
                      {teachers.length > 0 && (
                        <Button
                          ...
                          title="Unduh format template terisi seluruh data GTK yang ada di database saat ini"
                        >
                          <Download className="w-3.5 h-3.5" /> Terisi Data GTK ({teachers.length})
                        </Button>
                      )}
                      <Button
                        type="button"
                        ...
                        className="bg-white hover:bg-slate-100 text-slate-700 border-slate-300 font-bold rounded-xl text-xs gap-1.5 shadow-sm"
                      >
                              ...
                              return (
                                <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                                  ...
                                  <td className="py-2 px-3">
                                    ...
                                    {(item.data.tempat_lahir || item.data.tanggal_lahir) && (
                                      <div className="text-[10px] text-slate-500 font-medium mt-0.5">
                                        TTL: {item.data.tempat_lahir || '-'}{item.data.tanggal_lahir ? `, ${item.data.tanggal_lahir}` : ''}
                                      </div>
                                    )}
                                  </td>
        ...
        <GoogleSheetsSyncModal
          ...
          onSyncComplete={(syncedData) => {
            setTeachers(syncedData);
            saveTeachersToDb(syncedData);
          }}
        />
      </div>
    </AdminLayout>
