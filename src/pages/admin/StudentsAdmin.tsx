...
import { loadPersistedClasses, savePersistedClasses, loadPersistedStudents, savePersistedStudents } from '@/utils/rombelPersistence';
import * as XLSX from 'xlsx';
import { compressImage } from '@/utils/imageCompression';
import { downloadStudentTemplateWithDropdowns } from '@/utils/excelTemplateHelper';
import { normalizeDateToYMD } from '@/utils/gsheetParserHelper';
...
const StudentsAdmin = () => {
  ...
  const sortedClassesList = useMemo(() => {
    ...
    const classMap = new Map<string, Kelas>();
    sourceClasses.forEach(c => {
      if (!c) return;
      const key = (c.nama_kelas || c.tingkat || c.id || '').toString().toLowerCase().trim();
      if (!key) return;
      if (!classMap.has(key)) {
        classMap.set(key, c);
      } else {
        const existing = classMap.get(key);
        if (existing.tahun_pelajaran !== targetYear && c.tahun_pelajaran === targetYear) {
          classMap.set(key, c);
        }
      }
    });
  ...
  const notifyAndSyncStudentData = async (newList: Student[], showToast = false) => {
    ...
    await savePersistedStudents(newList, studentsKey);
  ...
  const fetchData = async (isSilent = false) => {
    ...
    try {
      ...
      const loadedPersistedStudents = await loadPersistedStudents(studentsKey);
      if (loadedPersistedStudents && loadedPersistedStudents.length > 0) {
        studentData = loadedPersistedStudents;
      }
      if (!studentData) studentData = [];
      const classData = await loadPersistedClasses(classesKey);
  ...
  const downloadTemplate = async () => {
    try {
      await downloadStudentTemplateWithDropdowns(
        `Template_Import_Siswa_${activeMadrasah?.nama_madrasah || 'Madrasah'}`,
        classes
      );
      showSuccess('Template Excel Siswa berhasil diunduh dengan proteksi teks angka 16-digit NISN/NIK & dropdown!');
    } catch {
      const headers = [
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        const objectRows = XLSX.utils.sheet_to_json(worksheet, { raw: false, defval: '' }) as any[];
        // 2. Try Array-based Sheet Parsing (Fallback for Headerless / Fixed Column Files)
        const arrayRows = XLSX.utils.sheet_to_json(worksheet, { header: 1, raw: false, defval: '' }) as any[][];
        ...
        // Helper to format numeric strings (like NISN / NIK) without scientific notation or trailing zero corruption
        ...
        // Helper to format date cells (e.g. XLSX date numbers, Indonesian dates, or strings)
  ...
  if (isPrinting) {
          ...
          <style dangerouslySetInnerHTML={{ __html: `
            @media print {
              ...
              *, *::before, *::after { box-sizing: border-box !important; }
              html, body { background: white !important; color: black !important; margin: 0 !important; padding: 0 !important; width: 100% !important; }
    ...
    return (
      <div className="min-h-screen bg-white p-0">
        ...
        <style dangerouslySetInnerHTML={{ __html: `
          @media print {
            @page { size: ${isLandscape ? 'A4 landscape' : 'A4 portrait'}; margin: 8mm 6mm; }
            *, *::before, *::after { box-sizing: border-box !important; }
            html, body { background: white !important; color: black !important; margin: 0 !important; padding: 0 !important; width: 100% !important; }
            .print\\:hidden { display: none !important; }
            table { width: 100% !important; border-collapse: collapse !important; page-break-inside: auto !important; }
