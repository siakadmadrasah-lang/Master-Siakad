...
import { 
  ...
  Info, Archive, Search, Calendar as CalendarIcon, Sparkles
} from 'lucide-react';
...
const ExamCardGenerator: React.FC = () => {
  ...
  const defaultOfficialName = getOfficialMadrasahName(activeMadrasah, settings.identitas_madrasah, "MADRASAH IBTIDAIYAH NEGERI");

  // Global Settings
  const [temaWarna, setTemaWarna] = useState<'blue' | 'orange' | 'green'>('blue');
  const [judulKartu, setJudulKartu] = useState("KARTU PESERTA TKAD");
  const [lembagaNama, setLembagaNama] = useState(defaultOfficialName);
  const [lembagaLogo, setLembagaLogo] = useState<string>("");

  // Pengaturan Kop Surat (Sinkronisasi Otomatis dari Modul Kop Surat)
  const [headerPengayom, setHeaderPengayom] = useState<string>("KEMENTERIAN AGAMA REPUBLIK INDONESIA");
  const [namaYayasan, setNamaYayasan] = useState<string>("");
  const [subHeader, setSubHeader] = useState<string>("");
  const [alamatKop, setAlamatKop] = useState<string>("");
  const [kontakKop, setKontakKop] = useState<string>("");
  const [logoKananUrl, setLogoKananUrl] = useState<string>("");
  const [garisStyle, setGarisStyle] = useState<'double' | 'single' | 'thick' | 'accent' | 'none'>('double');
  ...
  // Fungsi sinkronisasi otomatis dari Modul Kop Surat
  const syncFromKopSurat = (showToast = true) => {
    ...
    const rawAlamat = identitas.alamat || activeMadrasah?.alamat || settings?.general?.address || '';
    ...
    const autoAlamat = [rawAlamat, kec ? `Kec. ${kec}` : '', kab ? (kab.toLowerCase().startsWith('kab') ? kab : `Kab. ${kab}`) : ''].filter(Boolean).join(', ');

    const telp = identitas.telepon || activeMadrasah?.telepon || settings?.general?.phone || '';

    setHeaderPengayom(identitas.header_pengayom || 'KEMENTERIAN AGAMA REPUBLIK INDONESIA');
    setNamaYayasan(identitas.nama_yayasan_kop || identitas.nama_yayasan || (activeMadrasah as any)?.nama_yayasan || '');
    setLembagaNama(identitas.nama_kop || (activeMadrasah as any)?.nama_resmi || identitas.nama_resmi || identitas.nama_madrasah || activeMadrasah?.nama_madrasah || defaultOfficialName);
    setSubHeader(identitas.sub_header_kop || autoSubHeader);
    setAlamatKop(identitas.alamat_kop || autoAlamat);
    setKontakKop(identitas.kontak_kop || (telp ? `Telp: ${telp}` : ''));
    setLembagaLogo(identitas.logo_url || activeMadrasah?.logo_url || settings?.general?.logo_url || '');
    setLogoKananUrl(identitas.logo_kanan_url || '');
    setGarisStyle(identitas.garis_style_kop || 'double');

    if (showToast) {
      showSuccess("Kop kartu ujian berhasil disesuaikan dengan Modul Kop Surat!");
    }
  };
    ...
    try {
      ...
      if (res?.value) {
        ...
        if (saved.settings) {
          ...
          setLembagaNama(saved.settings.lembagaNama || defaultOfficialName);
          setLembagaLogo(saved.settings.lembagaLogo || "");
          if (saved.settings.headerPengayom !== undefined) setHeaderPengayom(saved.settings.headerPengayom);
          if (saved.settings.namaYayasan !== undefined) setNamaYayasan(saved.settings.namaYayasan);
          if (saved.settings.subHeader !== undefined) setSubHeader(saved.settings.subHeader);
          if (saved.settings.alamatKop !== undefined) setAlamatKop(saved.settings.alamatKop);
          if (saved.settings.kontakKop !== undefined) setKontakKop(saved.settings.kontakKop);
          if (saved.settings.logoKananUrl !== undefined) setLogoKananUrl(saved.settings.logoKananUrl);
          if (saved.settings.garisStyle) setGarisStyle(saved.settings.garisStyle);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>, type: 'logo' | 'logo_kanan' | 'signature' | 'photo') => {
    ...
    reader.onloadend = () => {
      ...
      else if (type === 'logo_kanan') setLogoKananUrl(base64String);
                  ...
                  <Card>
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        <div>
                          <CardTitle>Kop Surat & Desain Kartu</CardTitle>
                          <CardDescription>Otomatis diselaraskan dengan Modul Kop Surat madrasah.</CardDescription>
                        </div>
                        <Button 
                          type="button" 
                          size="sm" 
                          variant="outline" 
                          className="bg-emerald-50 border-emerald-300 text-emerald-800 hover:bg-emerald-100 text-xs shrink-0"
                          onClick={() => syncFromKopSurat(true)}
                        >
                          <RefreshCw className="w-3.5 h-3.5 mr-1 text-emerald-600" />
                          Sesuaikan dg Kop Surat
                        </Button>
                      </div>
                    </CardHeader>
                    <CardContent className="space-y-6">
                      <div className="p-3 bg-emerald-50/70 border border-emerald-200 rounded-xl flex items-center justify-between gap-3 text-xs text-emerald-800">
                        <div className="flex items-center gap-2">
                          <Sparkles className="w-4 h-4 text-emerald-600 shrink-0" />
                          <span>Kop kartu ujian otomatis menarik data Pengayom, Yayasan, Nama Resmi Madrasah, NSM/NPSN, Alamat & Logo dari Modul Kop Surat.</span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div className="space-y-1">
                            <Label className="text-xs">Header Pengayom (Kop Baris 1)</Label>
                            <Input 
                              value={headerPengayom} 
                              onChange={(e) => setHeaderPengayom(e.target.value)}
                              placeholder="KEMENTERIAN AGAMA REPUBLIK INDONESIA"
                              className="text-xs"
                            />
                          </div>
                          <div className="space-y-1">
                            <Label className="text-xs">Nama Yayasan (Kop Baris 2 - Opsional)</Label>
                            <Input 
                              value={namaYayasan} 
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between">
                            <Label>Nama Lembaga / Satuan Pendidikan (Kop Utama)</Label>
                            {defaultOfficialName && (
                              <button
                                type="button"
                                onClick={() => setLembagaNama(defaultOfficialName)}
                                className="text-[11px] text-emerald-700 hover:underline font-bold"
                              >
                                Samakan dg Nama Resmi Madrasah
                              </button>
                            )}
                          </div>
                          <Input 
                        ...
                        <div className="space-y-1">
                          <Label className="text-xs">Sub-Header (NSM, NPSN, Akreditasi)</Label>
                          <Input 
                            value={subHeader} 
                            onChange={(e) => setSubHeader(e.target.value)}
                            placeholder="NSM: ... | NPSN: ... | AKREDITASI A"
                            className="text-xs"
                          />
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div className="space-y-1">
                            <Label className="text-xs">Alamat Kop</Label>
                            <Input 
                              value={alamatKop} 
                          ...
                          <div className="space-y-1">
                            <Label className="text-xs">Kontak Kop</Label>
                            <Input 
                              value={kontakKop} 
                              onChange={(e) => setKontakKop(e.target.value)}
                              placeholder="Telp: (021)... | Email: ..."
                              className="text-xs"
                            />
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {/* Logo Kiri */}
                          <div className="space-y-2 p-3 bg-gray-50/80 rounded-xl border border-gray-100">
                            <Label className="text-xs font-bold text-gray-700">Logo Lembaga (Kiri)</Label>
                            <div className="flex items-center gap-3">
                              <div className="w-12 h-12 border rounded-lg bg-white flex items-center justify-center overflow-hidden shrink-0">
                                {lembagaLogo ? (
                              ...
                              <div className="flex-1 space-y-1.5">
                                ...
                                <div className="flex gap-1.5">
                                  <Button size="sm" variant="outline" className="text-xs h-8" onClick={() => logoInputRef.current?.click()}>
                                    <Upload className="w-3.5 h-3.5 mr-1" /> Unggah
                                  </Button>
                                  {lembagaLogo && (
                                    <Button size="sm" variant="ghost" className="text-red-500 text-xs h-8 px-2" onClick={() => setLembagaLogo("")}>
                          ...
                          {/* Logo Kanan */}
                          <div className="space-y-2 p-3 bg-gray-50/80 rounded-xl border border-gray-100">
                            <Label className="text-xs font-bold text-gray-700">Logo Tambahan (Kanan - Opsional)</Label>
                            <div className="flex items-center gap-3">
                              <div className="w-12 h-12 border rounded-lg bg-white flex items-center justify-center overflow-hidden shrink-0">
                                {logoKananUrl ? (
                                  <img src={logoKananUrl} alt="Logo Kanan" className="w-full h-full object-contain" />
                                ) : (
                                  <ImageIcon className="w-6 h-6 text-gray-200" />
                                )}
                              </div>
                              <div className="flex-1 space-y-1.5">
                                <input 
                                  ...
                                  accept="image/*" 
                                  onChange={(e) => handleFileUpload(e, 'logo_kanan')} 
                                />
                                <div className="flex gap-1.5">
                                  <Button size="sm" variant="outline" className="text-xs h-8" onClick={() => logoKananInputRef.current?.click()}>
                                    <Upload className="w-3.5 h-3.5 mr-1" /> Unggah
                                  </Button>
                                  {logoKananUrl && (
                                    <Button size="sm" variant="ghost" className="text-red-500 text-xs h-8 px-2" onClick={() => setLogoKananUrl("")}>
                                      <X className="w-3.5 h-3.5" />
                                    </Button>
                                  )}
                                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div className="space-y-1">
                            <Label className="text-xs">Gaya Garis Pembatas Kop</Label>
                            <Select value={garisStyle} onValueChange={(val: any) => setGarisStyle(val)}>
                              <SelectTrigger className="text-xs h-9">
                                <SelectValue placeholder="Pilih gaya garis" />
                              </SelectTrigger>
                              <SelectContent>
                                <SelectItem value="double">Ganda Standar (Resmi Kemenag)</SelectItem>
                                <SelectItem value="single">Tunggal Tipis</SelectItem>
                                <SelectItem value="thick">Tebal Tunggal</SelectItem>
                                <SelectItem value="accent">Warna Aksen</SelectItem>
                          ...
                          <div className="space-y-1">
                            <Label className="text-xs">Tema Warna Utama</Label>
                            <div className="flex gap-2">
                              {['blue', 'orange', 'green'].map((color) => (
                                <button
                                  ...
                                  className={`flex-1 h-9 rounded-lg border-2 transition-all flex items-center justify-center gap-1.5 ${
                                    temaWarna === color 
                                ...
                                >
                                  ...
                                  <span className="text-[11px] font-bold capitalize">{color}</span>
                                </button>
                              ))}
                            </div>
                          </div>
