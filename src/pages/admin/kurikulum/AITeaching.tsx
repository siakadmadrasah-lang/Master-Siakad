...
const AITeaching = () => {
  ...
  const officialSchoolName = getOfficialMadrasahName(activeMadrasah, settings.identitas_madrasah, settings.general?.school_name || "Si@Kad");
  ...
  const fetchBedahCP = async () => {
    try {
      ...
      if (res?.value && Array.isArray(res.value)) {
        setBedahCPData(res.value);
      }
    } catch (err) {
      console.error('Error fetching bedah_cp_data in AITeaching:', err);
    }
  };
  ...
  const normalizeFase = (faseStr: string) => {
    if (!faseStr) return '';
    const match = faseStr.match(/Fase\s*([A-F])/i) || faseStr.match(/\b([A-F])\b/i);
    return match ? match[1].toUpperCase() : faseStr.trim().toUpperCase();
  };

  const availableMateri = useMemo(() => {
    const source = bedahCPData.length > 0 ? bedahCPData : (settings.bedah_cp_data || []);
    if (!source || !Array.isArray(source)) return [];

    const targetMapel = normalizeSubject(formData.mata_pelajaran);
    const targetFase = normalizeFase(formData.fase);

    let matched = source.filter((item: any) => {
      if (!item || !item.materi_pokok || typeof item.materi_pokok !== 'string' || !item.materi_pokok.trim()) return false;
      const itemMapel = normalizeSubject(item.mata_pelajaran);
      const itemFase = normalizeFase(item.fase);

      const matchMapel = itemMapel === targetMapel || itemMapel.includes(targetMapel) || targetMapel.includes(itemMapel);
      const matchFase = !itemFase || itemFase === targetFase;
      return matchMapel && matchFase;
    });

    // Fallback if no match for exact fase: match mapel only
    if (matched.length === 0 && targetMapel) {
      matched = source.filter((item: any) => {
        if (!item || !item.materi_pokok || typeof item.materi_pokok !== 'string' || !item.materi_pokok.trim()) return false;
        const itemMapel = normalizeSubject(item.mata_pelajaran);
        return itemMapel === targetMapel || itemMapel.includes(targetMapel) || targetMapel.includes(itemMapel);
      });
    }

    // Deduplicate by materi_pokok
    const uniqueMateriMap = new Map<string, any>();
    matched.forEach((item: any) => {
      const key = item.materi_pokok.trim().toLowerCase();
      if (!uniqueMateriMap.has(key)) {
        uniqueMateriMap.set(key, item);
      }
    });

    return Array.from(uniqueMateriMap.values());
  }, [bedahCPData, settings.bedah_cp_data, formData.mata_pelajaran, formData.fase]);
...
- Siswa yang sudah tuntas dapat mengikuti **PENGAJAAN**`;
  ...
  const handleGenerateAI = () => {
    ...
    setTimeout(() => {
      ...
      const schoolName = officialSchoolName;
                    ...
                    <div className="flex flex-col h-full justify-between">
                      ...
                      <div className="flex flex-col items-center">
                        ...
                        <div className="w-full text-center font-serif border-t-2 border-black pt-4">
                          <p className="text-[14pt] font-bold uppercase leading-tight">{officialSchoolName}</p>
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <Button onClick={() => { 
            ...
            fetchBedahCP();
            setDialogOpen(true); 
          }} className="bg-emerald-600 text-white rounded-xl font-bold shadow-lg">
        ...
        <DialogContent className="sm:max-w-3xl rounded-3xl max-h-[calc(100dvh-40px)] overflow-y-auto">
          ...
          <div className="space-y-6 pt-4">
            ...
            <div className="space-y-3">
              <label className="text-xs font-bold text-emerald-600 uppercase flex items-center justify-between">
                <span>Pilih Materi Pokok (Dari Bedah CP)</span>
                {availableMateri.length > 0 && (
                  <span className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full font-bold">
                    {availableMateri.length} Materi Ditemukan
                  </span>
                )}
              </label>
                  ...
                  {availableMateri.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {availableMateri.map((m: any, idx: number) => (
                        <div key={idx} className="flex items-start space-x-2 bg-white p-2.5 rounded-xl border shadow-sm hover:border-emerald-300 transition-colors">
                          <Checkbox id={`materi-${idx}`} checked={formData.materi_pokok.includes(m.materi_pokok)} onCheckedChange={() => toggleMateri(m.materi_pokok)} className="mt-0.5" />
                          <label htmlFor={`materi-${idx}`} className="text-xs font-medium cursor-pointer flex-1">
                            {m.materi_pokok}
                            {m.elemen && <span className="block text-[10px] text-emerald-600 font-semibold mt-0.5">Elemen: {m.elemen}</span>}
                          </label>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="text-center py-6 space-y-1">
                      <p className="text-xs text-gray-500 font-medium">Data Bedah CP belum tersedia untuk <span className="font-bold text-emerald-700">{formData.mata_pelajaran}</span> ({formData.fase}).</p>
                      <p className="text-[11px] text-gray-400">Silakan tambahkan data di menu <span className="font-semibold text-gray-600">Kurikulum &gt; Bedah CP</span>.</p>
                    </div>
                  )}
                </ScrollArea>
              </Card>
