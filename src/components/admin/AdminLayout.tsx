...
const AdminLayout: React.FC<AdminLayoutProps> = ({ children, title }) => {
  ...
  const menuGroups = [
    ...
    {
      key: 'gtk',
      label: 'Modul Data GTK & Guru',
      icon: UserCheck,
      items: [
        { icon: LayoutDashboard, label: 'Pusat Kendali Ruang Guru', path: '/admin/ruang-guru' },
    ...
    },
    {
      ...
      items: [
        ...
        { icon: HeartHandshake, label: 'Laporan Pembiasaan', path: '/admin/pembiasaan' },
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex overflow-x-clip max-w-full w-full print:bg-white">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-20 bg-white/95 backdrop-blur-xl border-b border-slate-200/90 flex items-center justify-between px-4 lg:px-8 sticky top-0 z-40 shadow-sm print:hidden">
        ...
        <main className="flex-1 p-3 sm:p-4 lg:p-8 max-w-full overflow-x-clip min-w-0 pb-20 lg:pb-8 print:p-0">
          ...
          {activeGroup && activeGroup.items.length > 1 && (
            <div className="mb-6 bg-white p-3 sm:p-4 rounded-3xl shadow-md border border-slate-200/80 transition-all print:hidden w-full overflow-hidden">
              <div className="flex items-center justify-between gap-2 mb-3 px-1">
                <div className="flex items-center gap-2 sm:gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <span className="text-[10px] font-black text-emerald-600 uppercase tracking-wider block truncate">
                      Navigasi Sub-Modul
                    </span>
                    <h3 className="text-xs sm:text-sm font-extrabold text-slate-900 leading-none truncate">
                      {activeGroup.label}
                    </h3>
                  </div>
                </div>
                <Badge className="bg-emerald-50 text-emerald-800 border border-emerald-200/80 text-[10px] font-black px-2.5 py-1 rounded-full shrink-0">
                  {activeGroup.items.length} Menu Modul
                </Badge>
              </div>

              {/* Grid Layout (Responsive on mobile) for Sub-modules */}
              <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-2 sm:gap-2.5 pt-0.5">
                {activeGroup.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2 sm:gap-2.5 px-3 py-2 sm:py-2.5 rounded-2xl transition-all min-w-0 border cursor-pointer ${
                        itemActive
                    ...
                    >
                      ...
                      <span className="whitespace-normal leading-tight font-black text-left flex-1 break-words text-[11px] sm:text-xs min-w-0">
                        {item.label}
                      </span>
                    </button>
                  );
