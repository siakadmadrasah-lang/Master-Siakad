...
export interface MadrasahItem {
  ...
  nama_resmi?: string; // Nama Resmi Madrasah / Nama Satuan Pendidikan Resmi Pada Semua Dokumen
  nsm: string;
  npsn: string;
...
/**
 * Helper untuk mengambil Nama Resmi Madrasah / Satuan Pendidikan secara konsisten
 * Memprioritaskan nama_resmi dari activeMadrasah atau identitas_madrasah
 */
export const getOfficialMadrasahName = (
  madrasah?: Partial<MadrasahItem> | null,
  identitas?: any,
  fallback: string = 'Madrasah'
): string => {
  return (
    madrasah?.nama_resmi?.trim() ||
    identitas?.nama_resmi?.trim() ||
    madrasah?.nama_madrasah?.trim() ||
    identitas?.nama_madrasah?.trim() ||
    fallback
  );
};
...
export const defaultMadrasahs: MadrasahItem[] = [
  ...
  },
  {
    ...
    nama_resmi: 'MADRASAH TSANAWIYAH AL-IKHLAS',
    nsm: '121234567891',
    npsn: '60723457',
...
export const MadrasahProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('siakad_madrasah_updated', handleSync);
    window.addEventListener('siakad_identitas_updated', handleSync);
    window.addEventListener('siakad_settings_updated', handleSync);
    window.addEventListener('storage', handleSync);

    return () => {
      window.removeEventListener('siakad_madrasah_updated', handleSync);
      window.removeEventListener('siakad_identitas_updated', handleSync);
      window.removeEventListener('siakad_settings_updated', handleSync);
